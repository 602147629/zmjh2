	...
	public class SkillView extends BaseView
	{
		...
		private function initUI():void{
			...
			for(var i:int = 1; i < 6; i++){
				(layer["Learn" + i] as SimpleButton).visible = false;
		...
		private function changeRoleInfo():void{
			...
			for(var j:int = 1; j < 3; j++){
				((layer["KungFuImg_" + j] as Sprite).getChildAt(0) as BaseNativeEntity).data.bitmapData = AUtils.getNewObj(player.fodder + "KungFu" + j) as BitmapData;
			}
		}
		...
		private function skillTypeSetting() : void{
			...
			if(player.skill.kungfu1 < 5){
				(layer["KungFuSoul1"] as TextField).text = _skillInfo[player.skill.kungfu1].soul;
			}else{
				(layer["KungFuSoul1"] as TextField).text = "";
			}
			if(player.skill.kungfu2 < 5){
				(layer["KungFuSoul2"] as TextField).text = _skillInfo[player.skill.kungfu2 + 5].soul;
			}else{
		...
		private function skillInfoSetting() : void{
			...
			for(var j:int = 0; j < _skillIconList.length; j++){
				((_skillIconList[j] as Sprite).getChildAt(0) as BaseNativeEntity).data.bitmapData = AUtils.getNewObj(player.fodder + "SkillIcon" + ((j + 1) + _skillType * 5)) as BitmapData;
			}
			skillInfoShow();
		}
		...
		private function skillLevelBtnShow() : void{
			if(_skillType == 0){
				...
				if(player.skill.kungfu1 < 5 && player.soul < _skillInfo[player.skill.kungfu1].soul){
				...
				if(player.skill.kungfu2 < 5 && player.soul < _skillInfo[player.skill.kungfu2 + 5].soul){
		...
		private function onLevelUp(e:MouseEvent) : void{
			...
			PlayerManager.getIns().addSoul(-_skillInfo[player.skill["kungfu" + index]].soul);
			player.skill["kungfu" + index]++;
			player.skill.skillArr[player.skill["kungfu" + index] - 1 + 5 * _skillType] = 1;
			skillInfoShow();
			update();
		...
		override public function update():void{
			(layer["RoleName"] as MovieClip).gotoAndStop(player.occupation);
			(layer["AllSoul"] as TextField).text = player.soul.toString();
			skillTypeSetting();
			skillInfoSetting();
			skillLevelBtnShow();
			skillLightShow();
