package com.test.game.UI.Grid
{
	...
	public class BaGuaAutoGrid extends Sprite
	{
		...
		public function BaGuaAutoGrid(CS:Class,col:uint, row:uint, W:int, H:int, offsetX:int, offsetY:int):void
		{
			_CS = CS;
			_col = col;
			_row = row;
		...
		private function initRender() : void
		{
			...
			for (var i:int = 0; i <totalIconNum ; i++)
			{
				...
				}else if(i<PlayerManager.getIns().player.baGuaRoomMax){
					item.setData(null);
				}else{
					item.setLocked();
				}
