package com.test.game.Mvc.BmdView
{
	...
	import com.test.game.Mvc.control.character.EscortAutoFightControl;
	
	import flash.display.Sprite;
	import flash.events.KeyboardEvent;
	import flash.utils.ByteArray;
	
	public class LootSceneView extends BaseBmdView
	{
		...
		private var _liv:LevelInfoView;
		private var _biaoChePosition:Sprite;
		public function LootSceneView()
		{
			super();
		}
		...
		private function initParams():void{
			...
			EscortManager.getIns().isGameOver = false;
			_liv = (ViewFactory.getIns().getView(LevelInfoView) as LevelInfoView);
			initUI();
			RoleManager.getIns().fightType = 1;
		}
		
		private function initUI() : void{
			_biaoChePosition = AssetsManager.getIns().getAssetObject("BiaoChePosition") as Sprite;
			_biaoChePosition.x = 50;
			_biaoChePosition.y = 370;
		}
		...
		public function initPlayer():void{
			...
			data.data = null;
			initMyPlayer(RoleManager.getIns().createPlayer(4400, 400, data));
			initEnemyData();
			PlayerManager.getIns().updatePropertys();
			(ViewFactory.getIns().getView(EscortMapView) as EscortMapView).x = -3700;
		...
		private function initEnemyData() : void{
			var convoysData:Object = EscortManager.getIns().gainData;
			var data:Object = new Object();
			data.uid = "1234567890";
			data.data = convoysData.otherPlayerData;
			var posX:int = (int(convoysData.escortData.carX)==0?240:convoysData.escortData.carX);
			initEnemyPlayer(RoleManager.getIns().createPlayer(posX, 400, data, 4));
			var biaoCheName:String;
			var fodder:String;
			switch(convoysData.otherPlayerData.EscortType){
				case 1:
					EscortManager.getIns().biaoCheSpeed = .5;
					initConvoy(RoleManager.getIns().createConvoy(5901, posX, 350, convoysData.otherPlayerData.lv));
					initConvoy(RoleManager.getIns().createConvoy(5902, posX + 50, 400, convoysData.otherPlayerData.lv));
					biaoCheName = "木牛";
					fodder = "BiaoCheMuNiu";
					break;
				case 2:
					EscortManager.getIns().biaoCheSpeed = .55;
					initConvoy(RoleManager.getIns().createConvoy(5911, posX, 350, convoysData.otherPlayerData.lv));
					initConvoy(RoleManager.getIns().createConvoy(5912, posX + 50, 400, convoysData.otherPlayerData.lv));
					initConvoy(RoleManager.getIns().createConvoy(5913, posX, 450, convoysData.otherPlayerData.lv));
					biaoCheName = "流马";
					fodder = "BiaoCheLiuMa";
					break;
				case 3:
					EscortManager.getIns().biaoCheSpeed = .6;
					initConvoy(RoleManager.getIns().createConvoy(5921, posX, 350, convoysData.otherPlayerData.lv));
					initConvoy(RoleManager.getIns().createConvoy(5922, posX + 50, 400, convoysData.otherPlayerData.lv));
					initConvoy(RoleManager.getIns().createConvoy(5923, posX, 450, convoysData.otherPlayerData.lv));
					biaoCheName = "金车";
			...
			(ViewFactory.getIns().getView(EscortBar) as EscortBar).escortSetting(biaoCheName, convoysData.otherPlayerData.lv, fodder, allHp, convoysData.otherPlayerData.EscortType);
			
			var nowHp:int = convoysData.escortData.carHp;
			nowHp = (nowHp==0?allHp:nowHp);
			if(nowHp > _convoys[0].charData.totalProperty.hp){
				_convoys[0].charData.useProperty.hp = _convoys[0].charData.totalProperty.hp;
				nowHp -= _convoys[0].charData.totalProperty.hp;
				if(nowHp > _convoys[1].charData.totalProperty.hp){
					_convoys[1].charData.useProperty.hp = _convoys[1].charData.totalProperty.hp;
					nowHp -= _convoys[1].charData.totalProperty.hp;
					if(_convoys.length >= 3){
						_convoys[2].charData.useProperty.hp = nowHp;
					}
				}else{
					_convoys[1].charData.useProperty.hp = nowHp;
				}
			}else{
				_convoys[0].charData.useProperty.hp = nowHp;
			}
			var time:int = int(convoysData.escortData.carTime) * 30;
			time = (time==0?90:time);
			(ViewFactory.getIns().getView(LevelInfoView) as LevelInfoView).levelTimeEffect.secStep = time;
			(ViewFactory.getIns().getView(LevelInfoView) as LevelInfoView).levelTimeEffect.oppositeType = true;
		}
		...
		private function initConvoy(con:EscortEntity):void{
			...
			con.collisionIndex = CollisionFilterIndexConst.MONSTER;
			con.collisionListeners = [CollisionFilterIndexConst.PLAYER_SKILL, CollisionFilterIndexConst.ALL_SKILL];
		...
		private function initEnemyPlayer(otherPlayer:PlayerEntity) : void{
			...
			_otherPlayer.charData.useProperty.hp = (convoysData.escortData.playerHp==0?_otherPlayer.charData.totalProperty.hp:convoysData.escortData.playerHp)
			_otherPlayer.autoFightControl = new EscortAutoFightControl(_otherPlayer);
		...
		public function playerDeath(player:PlayerEntity) : void{
			if(_myPlayer.charData.useProperty.hp <= 0){
				EscortManager.getIns().lootResultShow();
				EscortManager.getIns().lootTimeStop();
				allMonsterLock();
			}
		}
		...
		private var _arrowShow:Boolean = true;
		private var _arrowStep:int;
		private function arrowJudge():void{
			if(_myPlayer == null || convoys[0] == null) return;
			_arrowStep++;
			if(_arrowStep < 30 * 3)	return;
			if(_arrowShow){
				if(_myPlayer.x - convoys[0].x > 800){
					if(_arrowStep % 20 == 0){
						if(_biaoChePosition.parent != null){
							_biaoChePosition.parent.removeChild(_biaoChePosition);
						}
					}else if(_arrowStep % 10 == 0){
						LayerManager.getIns().gameTipLayer.addChild(_biaoChePosition);
					}
				}else{
					if(_biaoChePosition.parent != null){
						_biaoChePosition.parent.removeChild(_biaoChePosition);
		...
		private function timeJudge():void{
			if(_liv.levelTimeEffect.secStep == 0 && !EscortManager.getIns().isGameOver){
				EscortManager.getIns().lootResultShow();
				EscortManager.getIns().lootTimeStop();
				allPlayerLock();
			}
		...
		private function convoyMove():void{
			if(EscortManager.getIns().isGameOver)	return;
			var useHp:int = 0;
			var totalHp:int = 0;
		...
		override public function destroy():void{
			...
			_monsterResource.length = 0;
			_monsterResource = null;
			
			if(_biaoChePosition != null){
				if(_biaoChePosition.parent != null){
					_biaoChePosition.parent.removeChild(_biaoChePosition);
				}
				_biaoChePosition = null;
			}
