package com.test.game.Mvc.BmdView
{
	...
	public class FunnyBossSceneView extends BaseBmdView
	{
		...
		public function FunnyBossSceneView()
		{
			super();
		}
		...
		override public function init():void{
			...
			initMonster(FunnyBossManager.getIns().createMonster(), false);
		}
		...
		private function start():void{
			...
			data.data = null;
			initPlayer(RoleManager.getIns().createPlayer(200, 450, data));
			initPartnerPlayer(RoleManager.getIns().createPartnerPlayer(200, 350, data));
		...
		public function playerDeath(player:PlayerEntity) : void{
			...
			if(result == true){
				FunnyBossManager.getIns().funnyPlayerDeath();
				for each(var item:MonsterEntity in _monsterVec){
					item.isLock = true;
				}
			}
		...
		public function initMonster(mons:Vector.<MonsterEntity>, hasAppear:Boolean = true) : void{
			...
			for(var i:int = 0; i < mons.length; i++){
				...
				mon.characterControl.limitRightX = 950;
		...
		public function checkMonsterDeath(monster:MonsterEntity) : void{
			...
			monster.destroy();
			FunnyBossManager.getIns().funnyBossOver();
		}
		...
		protected function controlMap():void{
			...
			var map:FunnyMapView = (ViewFactory.getIns().getView(FunnyMapView) as FunnyMapView);
			if(map != null){
				map.moveMap();
			}
		}
