package com.test.game.Manager.Collision
{
	...
	public class PlayerKillingCollisionListener extends BaseCollisionListener
	{
		public function PlayerKillingCollisionListener()
		{
			super();
		}
		...
		override public function beforeCollision(child1:ICollisionAble,child2:ICollisionAble):Boolean{
			...
			if(child1 is SkillEntity){
				if(Math.abs(child1.collisionPos - child2.collisionPos) >= (child1 as SkillEntity).collisionRange)
					return false;
			}
			if(child2 is SkillEntity){
				if(Math.abs(child1.collisionPos - child2.collisionPos) >= (child2 as SkillEntity).collisionRange)
					return false;
			}
