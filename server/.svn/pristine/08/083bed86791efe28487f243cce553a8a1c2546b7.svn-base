...
var checkEscort = function(){
    ...
    if(EscortKeys.length > 0 && RobKeys.length>0){
        ...
        for(var i=0;i<arrTemp.length-1;i++){
            var obj1 = arrTemp[i];
            if(!waitingArr[obj1.gameKey] && !waitingRobArr[obj1.gameKey]){
                continue;
            }
            for(var j=i+1;j<arrTemp.length;j++){
                var obj2 = arrTemp[j];
                if(!waitingArr[obj2.gameKey] && !waitingRobArr[obj2.gameKey]){
                    continue;
                }
...
MatchEscortControllor.UpdateEscortData = function(obj,cb){
    ...
    if(waitingArr[obj.gameKey]){
        ...
        require("Log").trace("玩家:"+obj.gameKey+"护镖数据更新："+JSON.stringify(waitingArr[obj.gameKey].escortData));
    }else{
        require("Log").trace("玩家:"+obj.gameKey+"不在护镖队列里！");
    }
}
...
MatchEscortControllor.RemoveClient = function(gameKey,cb){
    if(waitingArr[gameKey]){
        ...
        require("Log").trace("该玩家都不在护镖劫镖匹配队伍当中！gameKey:"+gameKey);
    }
}
//根据uid获取玩家是否正在匹配
