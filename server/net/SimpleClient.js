...
function SimpleClient(socket) {
	...
	this.exBuffer = new ExBuffer(5120).uint32Head().littleEndian();
...
SimpleClient.prototype.onReceivePackData = function(buffer){
    ...
    var encodeType = buffer.readInt8(0);
    var isCrypt = buffer.readInt8(1);
//    require('Log').trace('----------------encodeType:'+encodeType);
//    require('Log').trace('----------------isCrypt:'+isCrypt);

    //解密
    this.decrypt(encodeType,buffer);
    //验证合法性
    if(isCrypt == 1){
...
SimpleClient.prototype.checkCrypt = function(buffer){
    if(this.type == 'RPC'){
        ...
        var crypt = buffer.readInt32LE(2);//客户端发来的加密值
        var cryptKey = buffer[6];//密钥的初始值(与客户端约定好,是包体数据中的第一个字节)
...
//加密
SimpleClient.prototype.encrypt = function(encodeType,buffer){

}
//解密
SimpleClient.prototype.decrypt = function(encodeType,buffer){
