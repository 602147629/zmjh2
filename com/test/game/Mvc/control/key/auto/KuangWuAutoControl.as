package com.test.game.Mvc.control.key.auto
{
	...
	public class KuangWuAutoControl extends BaseControl
	{
		...
		public static function getIns():KuangWuAutoControl{
			return Singleton.getIns(KuangWuAutoControl);
		}
		
		private var _playerKeyboard:PlayerKeyboardControl;
		private var _mapView:BaseMapView;
		private var _player:KuangWuEntity;
		...
		public function controlPlayer(player:PlayerEntity,playerId:uint):void{
			_player = player as KuangWuEntity;
			...
			//空中压制和地面压制
			if(canNotChangeSkillStatus() && _playerKeyboard.getPlayerKeyBoardByPlayer(_player.keyBoard,PlayerKeyboardControl.DOWN,playerId,KeyboardInput.KEYDOWN) && _player.characterControl.doubleJump != 0){
			...
			//第10个技能释放
			if(_playerKeyboard.getPlayerKeyBoardByPlayer(_player.keyBoard,PlayerKeyboardControl.SKILL_10,playerId,KeyboardInput.KEYUP) && _player.curAction == ActionState.SKILL10){
				_player.setAction(ActionState.SKILLOVER10);
				isAction = false;
			}
