package com.test.game.Entitys.Monsters
{
	...
	public class ChiNanEntity extends BaseMonsterEntity
	{
		private static const DEATHTIME:int = 60;
		private var _deathTime:int;
		public var isAlreadyDead:Boolean = false;
		public function ChiNanEntity(charVo:CharacterVo, hasMainAI:Boolean = true)
		{
			super(charVo, hasMainAI);
			this.characterJudge.hurtReduceType = ImmunityConst.ATK_IMMUNITY;
		...
		private function togetherReleaseSkill() : void{
			if(SceneManager.getIns().nowScene != null){
				...
				for(var i:int = 0; i < monsters.length; i++){
					if((monsters[i].charData as EnemyVo).ID == 2219
						|| (monsters[i].charData as EnemyVo).ID == 4219
						|| (monsters[i].charData as EnemyVo).ID == 6029){
